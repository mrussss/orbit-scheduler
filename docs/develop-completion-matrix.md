...
| O6C retention and secret boundary | Implemented | Trace stores bounded metadata summaries rather than issue text, error logs, source, raw tool arguments/results, model output, or credentials; policy is documented in `docs/agent-v0.3.md` | `TestExecutorTraceNeverPersistsPromptArgumentsOrToolResultContent`, size-bound tests, and smoke log credential scan |
...
| O8 Python compatibility baseline | Implemented | `labs/python-agent-baseline` contains typed Pydantic models, exceptions, async tools/loop, HTTP/OpenAI SDK shape, pytest, the same three schemas, and no LangGraph | its pytest suite drives all ten shared fixtures through `AgentRunner` and `SafeRepositoryTools` |
